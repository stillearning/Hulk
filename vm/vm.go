...
func (vm *VM) currentFrame() *Frame {
	return vm.frames[vm.framesInd-1]
}

func (vm *VM) pushFrame(f *Frame) {
	vm.frames[vm.framesInd] = f
	vm.framesInd++
}

func (vm *VM) popFrame() *Frame {
	vm.framesInd--
	return vm.frames[vm.framesInd]
}
...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpJumpNotTruthy:
			pos := int(code.ReadUint16(ins[ip+1:])) //pos is the next instruction to jump to if not truthy
